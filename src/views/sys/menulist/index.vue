<template>
  <div class="app-container">
    <h3 class="menu-header">系统菜单
      <el-button class="filter-item" style="margin-left: 50px;" type="primary" icon="el-icon-edit" @click="createDialog">添加顶级菜单</el-button>
    </h3>
    <el-tree
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="valids" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-if="checkItemId" label="父级" prop="parent">
...
import { getMenusList, addMenu, updateMenu, deleteMenu } from '@/api/menu'
export default {
  name: 'Sysmenu',
  directives: {
    waves
  ...
  data() {
    return {
      ...
      list: null,
      valids: {
        path: [{ required: true, message: '路径是必须的' }],
        title: [{ required: true, message: '名称是必须的' }],
        parent: [{ required: true, message: '父级必须设置' }]
      },
      checkItemId: null,
      menus: []
    }
  ...
  created() {
    this.menus.push({ key: 0, display_name: '顶层' })
    this.getList((list) => {
  ...
  methods: {
    ...
    formatTreeData(data) {
      if (!data) return
      data.map((item) => {
        this.menus.push({ key: item.id, display_name: item.meta.title })
